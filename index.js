...
const ADMIN_CHAT_ID = -5380192351;
...
bot.start((ctx) => {
  ...
  ctx.reply(
    ...
    Markup.keyboard([["✳️ Taklif", "❗️ Shikoyat", "💬 Fikr"]]).oneTime().resize()
  );
});
...
bot.hears(["✳️ Taklif", "❗️ Shikoyat", "💬 Fikr"], (ctx) => {
  ...
  ctx.reply(
    ...
    Markup.keyboard([["🏢 Uchtepa", "🏢 Sergeli"]]).oneTime().resize()
  );
});
...
bot.on("text", (ctx) => {
  ...
  if (!userData[userId].fullName) {
    userData[userId].fullName = ctx.message.text.toLowerCase() === "/skip" ? "Anonim" : ctx.message.text;

    ctx.reply(
      "Bu murojaat qaysi bo'lim uchun? Iltimos, tanlang",
      Markup.keyboard([["🏢 Ma’muriyat", "📚 O‘quv bo‘limi"]]).oneTime().resize()
    );
    return;
  }
  ...
  if (!userData[userId].department) {
    if (ctx.message.text === "🏢 Ma’muriyat" || ctx.message.text === "📚 O‘quv bo‘limi") {
      userData[userId].department = ctx.message.text;
      ctx.reply("Endi murojaatingiz matnini yozib yuboring:", Markup.removeKeyboard());
    } else {
  ...
  if (!userData[userId].group) {
    userData[userId].group = ctx.message.text.toLowerCase() === "/skip" ? "Ko‘rsatilmagan" : ctx.message.text;
    ...
    ctx.reply(
      ...
      Markup.keyboard([["✅ Ha, yuboring", "❌ Bekor qilish"]]).oneTime().resize()
    );
    return;
  }
...
app.post("/webhook", (req, res) => {
  res.sendStatus(200);
  bot.handleUpdate(req.body).catch((err) => console.error("Update error:", err));
});
